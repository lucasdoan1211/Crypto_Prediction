...
if st.button("Predict Next Day Close"):
    try:
        ...
        # Predict with Ridge (take first row and flatten for 1D input)
        X_ridge = X_selected[0, :].flatten()  # FIX: Take first row, then flatten
        ridge_prediction = ridge_model.predict(X_ridge)[0]
